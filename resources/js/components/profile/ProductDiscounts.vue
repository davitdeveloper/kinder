            ...
            <v-data-table :headers="headers" :items="discounts">
                <template v-slot:top>
                    <v-card tile flat class="border-bottom">
                        <v-toolbar flat>
                            <v-toolbar-title>Product Discounts</v-toolbar-title>
                            <v-spacer></v-spacer>
...
<script>
    ...
    export default {
        ...
        data: () => ({
            ...
            periodWidth: null,
            headers: [
                { text: "Model Title", value: "product.model.title" },
                { text: "Color", value: "product.color.name" },
                { text: "Size", value: "product.size.value" },
        ...
        methods: {
            ...
            getDiscounts () {
                axios.get(`/api/profile/product-discounts`)
                    .then(res => this.discounts = res.data,  )
                    .catch(err => console.error(err.response.data.message));
            },

            open(item, mode) {
                this.discount = new ProductDiscount(item);
                this.mode = mode;
                this.dialog = true;
            },
            ...
            save() {
                if (this.mode === 1) {
                    axios.post(`/api/profile/product-discount/create`, this.discount)
                        .then(res => {
                    ...
                    axios.put(`/api/profile/product-discount/update/${this.discount.id}`,this.discount)
                        .then(res => {
            ...
            remove (item) {
                ...
                axios.delete(`/api/profile/product-discount/delete/${item.id}`)
                    .then(res => {
                        let index = this.discounts.findIndex(discount => discount.id === res.data.id);
        ...
        watch: {
            search (val) {
                if (val) {
                    axios.get(`/api/profile/product-discount/search-product?search=${val}`)
                        .then(res => {
