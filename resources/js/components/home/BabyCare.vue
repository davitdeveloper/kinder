...
export default {
    ...
    data: () => ({
        skeleton: false,
        images: [
            { id: 1, src: "/images/boys_4.png", text: "0-3 Years", height: 80, margin: 1, active: true },
            { id: 2, src: "/images/girls_4.png", text: "3+ Years", height: 70, margin: 10, active: false },
        ],
        item: 0
    }),
    ...
    computed: {
        grandParent () {
            return this.$store.state.upperMenu.find(category => category.alias === "khnamq").alias;
        },

        parent () {
            return this.$store.state.upperMenu.find(category => category.alias === "khnamq").children[this.item].alias;
        },

        items () {
            return this.$store.state.upperMenu.find(category => category.alias === "khnamq").children[this.item].children;
        },

        products () {
            return this.$store.state.homeProducts.babyCare;
        }
    },
