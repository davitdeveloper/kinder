        ...
        <v-col v-if="$vuetify.breakpoint.smAndUp" cols="5" sm="4" md="3" lg="3" xl="3">
            <v-row no-gutters justify="center">
                <v-col
                    ...
                    class="text-center"
                >
                    <v-img :src="image.src" height="98.88" width="75.5" class="opacity" contain :class="{ active: image.active }" @click="makeActive(image)"></v-img>
                    <v-toolbar-title class="opacity" v-text="image.text" :class="{ active: image.active }" @click="makeActive(image)"></v-toolbar-title>
                </v-col>
            </v-row>
        </v-col>
                                    ...
                                    >
                                        <v-hover v-slot:default="{ hover }">
                                            <v-card :elevation="hover ? 1 : 0" tile outlined color="white" light @click="$router.push(`/product/${product.alias}`)">
                                                ...
                                                <v-card-text v-text="product.short_description"></v-card-text>
...
export default {
    ...
    data () {
        return {
            skeleton: false,
            images: [
                { id: 1, src: "/images/boys_1.png", text: "Տղաներ", active: true },
                { id: 2, src: "/images/girls_1.png", text: "Աղջիկներ", active: false },
                { id: 4, src: "/images/girls_1.png", text: "Աղջիկներ", active: false },
                { id: 5, src: "/images/girls_1.png", text: "Աղջիկներ", active: false },
                { id: 6, src: "/images/girls_1.png", text: "Աղջիկներ", active: false },
                { id: 7, src: "/images/girls_1.png", text: "Աղջիկներ", active: false },
                { id: 8, src: "/images/girls_1.png", text: "Աղջիկներ", active: false },
                { id: 9, src: "/images/girls_1.png", text: "Աղջիկներ", active: false },
            ],
            item: 0,
        }
    },
    ...
    computed: {
        grandParent () {
            return this.$store.state.upperMenu.find(category => category.alias === "noratsinner").alias;
        },

        parent () {
            return this.$store.state.upperMenu.find(category => category.alias === "noratsinner").children[this.item].alias;
        },

        items () {
            return this.$store.state.upperMenu.find(category => category.alias === "noratsinner").children[this.item].children;
        },

        products () {
            return this.$store.state.homeProducts.clothes0_3;
        }
    },
    ...
    methods: {
        ...
        makeFavorite(product) {
            ...
            if (!this.$store.state.auth.check) {
                ...
                axios.post(`/api/favorites/add-remove/${this.$store.state.auth.user.id}`, {
                    model_id: product.id
                }).then(res => {
                        this.$store.commit("updateFavorites", res.data.id)
