        ...
        <v-col v-if="$vuetify.breakpoint.smAndUp" cols="5" sm="4" md="3" lg="3" xl="3">
            <v-row no-gutters justify="center">
                <v-col
                    ...
                    class="text-center"
                >
                    <v-img :src="image.src" width="75.5" class="opacity" :class="{ active: image.active }" @click="makeActive(image)"></v-img>
...
export default {
    ...
    data: () => ({
        skeleton: false,
        images: [
            { id: 1, src: "/images/boys_2.png", text: "Տղաներ", active: true },
            { id: 2, src: "/images/girls_2.png", text: "Աղջիկներ", active: false },
        ],
        item: 0,
    }),
    ...
    computed: {
        grandParent () {
            return this.$store.state.upperMenu.find(category => category.alias === "poqr_erexaner").alias;
        },

        parent () {
            return this.$store.state.upperMenu.find(category => category.alias === "poqr_erexaner").children[this.item].alias;
        },

        items () {
            return this.$store.state.upperMenu.find(category => category.alias === "poqr_erexaner").children[this.item].children;
        },

        products () {
            return this.$store.state.homeProducts["clothes3+"];
        }
    },
